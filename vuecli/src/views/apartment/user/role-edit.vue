                        ...
                        <Row>
                            <Col :xs="24" :lg="12">
                                <FormItem label="模块" prop="moduleUuid">
                                    <Select v-model.trim="form.moduleUuid" placeholder="请选择模块..." disabled
                            ...
                            <!--<Col :xs="24" :lg="12">-->
                                <!--<FormItem label="实体" prop="clientUuid">-->
                                    <!--<Select v-model.trim="form.clientUuid" placeholder="请选择实体..." disabled>-->
...
<script>
    export default {
        ...
        activated () {
            this.activated();
            setTimeout(this.getData(this.$route.query.uuid), 1000);
        },
        data () {
            return {
                // 下拉列表
                searchModuleUuidTemp: '',
                moduleList: [],
                clientByModuleList: [],
                flag: false,
                form: {},
        ...
        methods: {
            activated () {
                this.searchModuleUuidTemp = this.searchModuleUuid;
                // 处理下拉列表
                this.$util.ajax.get('/uaa/module/GetModuleAll').then((res) => {
                    if (res.status === 200 && res.data.status === true) {
                        this.moduleList = res.data.content;
                        if (this.moduleList.length > 0 && this.searchModuleUuid === '') {
                            this.searchModuleUuid = this.moduleList[0].uuid;
                        } else {
                            this.searchModuleUuid = this.searchModuleUuidTemp;
                        }
                    }
                });
            },
            getData (uuid) {
                // 查询实体
                this.$util.ajax.get('/uaa/role/GetRoleByUuid', {params: {uuid: uuid}}).then((res) => {
                    if (res.status === 200 && res.data.status === true) {
                        this.form = res.data.content;
                        if (this.form.parentUuid !== '') {
                            this.flag = true;
                        }
                    }
                });
            ...
            handleSubmit (name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        this.form.clientUuid = this.searchClientUuid;
                        this.$util.ajax.post('/uaa/role/UpdateRole', this.form).then((res) => {
                            if (res.status === 200 && res.data.status === true) {
                                this.$Message.success('更新成功!');
                            } else {
                                if (res.data.error && res.data.error.length > 0) {
                                    this.$Message.error('更新失败!<br/>错误消息：' + res.data.error[0]);
                                } else {
